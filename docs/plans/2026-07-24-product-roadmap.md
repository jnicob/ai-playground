...
| B | API task-based propia (`POST /v1/services/{service}` → `task_id` → `GET /v1/tasks/{id}`) con OpenAPI + conectores server-side (pollinations, google) + adaptador `platform` + panel de keys pass-through | pendiente | A |
