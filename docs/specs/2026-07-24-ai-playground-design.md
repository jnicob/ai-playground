...
| Proveedores v1 | `mock` (default, client-side, determinista) · `pollinations` (sin key, tier gratuito) · `google` (Gemini API directa con key del usuario: imagen `gemini-2.5-flash-image`/nano banana con free tier ~500 img/día; vídeo Veo **opt-in con aviso explícito de coste** — sin free tier, $0.15–0.60/s) |
| API keys | Panel inline que pide la key al seleccionar un proveedor que la requiere; se guarda **por proveedor en `sessionStorage`** (dura la navegación); sin cuentas/login. La key solo viaja navegador → API del proveedor; jamás a bundle, repo o servidor propio |
